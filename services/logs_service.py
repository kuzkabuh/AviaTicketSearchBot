"""Сервис безопасного чтения логов бота и обновлений для административной панели."""

from __future__ import annotations

import asyncio
from dataclasses import dataclass
from pathlib import Path

from config import settings

MAX_TELEGRAM_PRE_TEXT = 3400
MAX_LINE_LENGTH = 220


@dataclass(frozen=True)
class LogView:
    """Результат чтения и фильтрации логов."""

    title: str
    text: str
    is_empty: bool = False
    path: str = ""
    truncated: bool = False


LOG_FILTERS: dict[str, tuple[str, tuple[str, ...], str]] = {
    "latest": ("🧾 <b>Последние записи в логе бота</b>", (), "Лог бота пуст."),
    "errors": (
        "❌ <b>Последние ошибки бота</b>",
        ("ERROR", "CRITICAL", "Traceback", "Exception"),
        "✅ За последнее время ошибок в логах не найдено.",
    ),
    "warnings": ("⚠️ <b>Последние предупреждения бота</b>", ("WARNING", "WARN"), "✅ За последнее время предупреждений в логах не найдено."),
    "subscriptions": (
        "🔔 <b>Логи подписок</b>",
        ("Subscription", "subscription", "подпис", "Price notification", "Tracked flight", "price check"),
        "✅ Событий по подпискам в логах не найдено.",
    ),
    "search": (
        "🔍 <b>Логи поиска билетов</b>",
        ("search", "Search", "ticket", "tickets", "билет", "маршрут", "API"),
        "✅ Событий поиска билетов в логах не найдено.",
    ),
}


def _read_lines(path: Path) -> tuple[list[str], bool]:
    if not path.exists():
        return [f"Файл лога не найден: {path}"], True
    if not path.is_file():
        return [f"Путь лога не является файлом: {path}"], True
    try:
        return path.read_text(encoding="utf-8", errors="replace").splitlines(), False
    except OSError as error:
        return [f"Не удалось прочитать файл лога: {error}"], True


def _trim_line(line: str) -> str:
    line = line.rstrip()
    if len(line) <= MAX_LINE_LENGTH:
        return line
    return f"{line[:MAX_LINE_LENGTH - 1]}…"


def _limit_text(lines: list[str], limit: int) -> tuple[str, bool]:
    truncated_by_lines = len(lines) > limit
    selected = [_trim_line(line) for line in lines[-limit:]]
    text = "\n".join(selected)
    if len(text) <= MAX_TELEGRAM_PRE_TEXT:
        return text, truncated_by_lines
    return f"…\n{text[-MAX_TELEGRAM_PRE_TEXT:]}", True


def _build_view(path: Path, title: str, empty_text: str, *, keywords: tuple[str, ...] = (), limit: int | None = None) -> LogView:
    lines, read_error = _read_lines(path)
    if keywords and not read_error:
        filtered = [line for line in lines if any(keyword.lower() in line.lower() for keyword in keywords)]
    else:
        filtered = lines

    if not filtered or (len(filtered) == 1 and filtered[0] == ""):
        return LogView(title=title, text=empty_text, is_empty=True, path=str(path))

    text, truncated = _limit_text(filtered, limit or settings.admin_log_lines_limit)
    return LogView(title=title, text=text, is_empty=read_error, path=str(path), truncated=truncated)


async def get_log_view(kind: str) -> LogView:
    """Возвращает подготовленный фрагмент логов для указанного раздела."""
    title, keywords, empty_text = LOG_FILTERS.get(kind, LOG_FILTERS["latest"])
    path = Path(settings.bot_error_log_path if kind == "errors" and settings.bot_error_log_path else settings.bot_log_path)
    return await asyncio.to_thread(_build_view, path, title, empty_text, keywords=keywords)


async def get_update_log_view() -> LogView:
    """Возвращает последние строки отдельного лога обновления."""
    path = Path(settings.bot_update_log_path)
    return await asyncio.to_thread(
        _build_view,
        path,
        "📋 <b>Последний лог обновления</b>",
        "Лог обновления пока пуст.",
        limit=settings.admin_log_lines_limit,
    )
