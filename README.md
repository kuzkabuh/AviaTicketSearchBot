...
- После выдачи результатов можно запросить календарные цены за периоды `±3 дня`, `Неделя` или `Месяц` от выбранной даты вылета.
...
- При создании подписки пользователь выбирает режим уведомлений: при любом изменении, только при снижении или ниже заданной суммы с вводом целевой цены.
...
11. После выдачи результатов бот спрашивает, нужно ли посмотреть цены рядом с выбранной датой, и предлагает кнопки `📅 ±3 дня`, `📆 Неделя`, `🗓 Месяц`. В выдаче календарных цен бот отмечает выбранную дату и самую низкую найденную цену; для месяца показывает топ самых дешёвых дат, чтобы не перегружать сообщение.
12. При нажатии `🔔 Отслеживать цену` бот спрашивает режим уведомлений: `🔄 При любом изменении`, `📉 Только при снижении`, `🎯 Ниже заданной суммы`. Первые два режима создают подписку сразу, а режим с заданной суммой запрашивает положительную целую целевую цену в рублях и сохраняет её в подписке.
...
migrations/003_subscription_notification_mode.sql # режим уведомлений подписки
migrations/004_subscription_target_price.sql # целевая цена подписки
```
