...
6. Пользователь вводит дату вылета в формате `YYYY-MM-DD`. Для варианта `🔁 Туда и обратно` на этом подэтапе дата возврата еще не запрашивается.
7. Бот спрашивает количество билетов и принимает только положительное целое число.
8. Бот показывает до `TICKET_RESULTS_LIMIT` вариантов и кнопку `🔔 Отслеживать цену` для каждого.
